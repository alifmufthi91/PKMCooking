...
public class RecipeDetail extends AppCompatActivity {
                                ...
                                if(myrate!=null){
                                    ...
                                    realm.executeTransaction(realm1 -> {
                                        ...
                                        if (olditem != null&&oldrating != null) {
                                            double newValue = (double) ratingBar.getRating();
                                            olditem.giveRating(newValue,oldValue);
                                            recipe_ratinggiver=olditem.getRating_giver();
                                            recipe_ratingvalue=olditem.getRating_value();
                                        }
                                    });
                                }else{
                                    realm.executeTransaction(realm1 -> {
                                        ...
                                        realm1.insert(rate);
                                        myrate = rate;
                                        ResepV2 item = realm1.where(ResepV2.class).equalTo("recipeId", recipeId).findFirst();
                                        if (item != null) {
    ...
    private void UpdateRate(Double value,int giver){
        RecipeDetail.this.runOnUiThread(new Runnable() {
            @Override
            public void run() {
